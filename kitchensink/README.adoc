...
== Deploy this application on OpenShift 4 using EAP operator

*Step 1: Install the EAP operator*

Login as a cluster administrator

Get a list of operators in the OpenShift marketplace

`oc get packagemanifests -n openshift-marketplace`

Add a subscription for EAP operator

`oc apply -f operator/subscription.yaml`

Verify that the subscription object was created

`oc get csv -n openshift-operators`

*Step 2: Deploy kitchensink application using this operator*

`oc apply -f .openshift/crd-eap-app.yaml`

*Step 3: Access the kitchensink application via the route*

Get the route hostname

`oc get route -n <your_namespace> --template='{{ .spec.host }}'`

Access this url via the browser and verify that the application is up and running.
